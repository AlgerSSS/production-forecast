...
function buildTimeSlotSheet(wb: any, opts: {
  ...
  const { timeSlotSuggestions, productSuggestions, timeslotSalesRecords, fixedSchedule, products, dailyTargets, selectedDate } = opts as any;
  const ws4 = wb.addWorksheet(`分时段_${selectedDate}`);
  const slotHeaders = ALL_SLOTS.map((s: string) => s.replace(":00", "点"));
  ...
  const currentDayTarget = dailyTargets.find((d: any) => d.date === selectedDate);
  ...
  const headerRow = ws4.addRow(["品名", "定位", "冷/热", "单价", "倍数", "满柜", "总数", "金额",
    ...slotHeaders.map((s: string) => `${s}出货`), ...slotHeaders.map((s: string) => `${s}金额`),
  ...
  // Build pivot data
  const productNames = [...new Set(productSuggestions.map((p: any) => p.productName))];
  const slotMap = new Map<string, Map<string, any>>();
  for (const s of timeSlotSuggestions) {
    if (!slotMap.has((s as any).productName)) slotMap.set((s as any).productName, new Map());
    slotMap.get((s as any).productName)!.set((s as any).timeSlot, s);
  }
  const productInfoMap = new Map<string, any>();
  for (const p of productSuggestions) productInfoMap.set((p as any).productName, p);
  const fullQtyMap = new Map<string, number>();
  for (const p of products) fullQtyMap.set((p as any).name, (p as any).displayFullQuantity);
  const priceMap = new Map<string, number>();
  for (const p of productSuggestions) priceMap.set((p as any).productName, (p as any).price);

  // Product rows
  for (const name of productNames) {
    const info = productInfoMap.get(name as string);
    const schedule = fixedSchedule[name as string] || [];
    const productSlots = (timeSlotSuggestions as any[]).filter((s: any) => s.productName === name);
    const totalQty = productSlots.reduce((sum: number, s: any) => sum + s.quantity, 0);
    const totalAmount = productSlots.reduce((sum: number, s: any) => sum + s.amount, 0);
    const slotQtyValues = ALL_SLOTS.map((slot) => { const data = slotMap.get(name as string)?.get(slot); return data && data.quantity > 0 ? data.quantity : ""; });
    const slotAmtValues = ALL_SLOTS.map((slot) => { const data = slotMap.get(name as string)?.get(slot); return data && data.amount > 0 ? data.amount : ""; });
    const row = ws4.addRow([name, info?.positioning ?? "", info?.coldHot ?? "", info?.price ?? "", info?.packMultiple ?? "", fullQtyMap.get(name as string) ?? "", totalQty, totalAmount, ...slotQtyValues, ...slotAmtValues, "", "", "", "", "", ""]);
  ...
  // 合计 row
  const sumSlotQtyValues = ALL_SLOTS.map((slot) => (timeSlotSuggestions as any[]).filter((s: any) => s.timeSlot === slot).reduce((sum: number, s: any) => sum + s.quantity, 0) || "");
  const sumSlotAmtValues = ALL_SLOTS.map((slot) => (timeSlotSuggestions as any[]).filter((s: any) => s.timeSlot === slot).reduce((sum: number, s: any) => sum + s.amount, 0) || "");
  const sumRow = ws4.addRow(["合计", "", "", "", "", "", (timeSlotSuggestions as any[]).reduce((s: number, item: any) => s + item.quantity, 0), (timeSlotSuggestions as any[]).reduce((s: number, item: any) => s + item.amount, 0), ...sumSlotQtyValues, ...sumSlotAmtValues]);
  ...
  const salesSlotValues = ALL_SLOTS.map((slot) => {
    ...
    const amt = Math.round((timeslotSalesRecords as any[]).filter((r: any) => r.timeSlot === slot).reduce((sum: number, r: any) => sum + r.avgQuantity * (priceMap.get(r.productName) ?? 0), 0));
  ...
  // 预计剩余 row (cumulative)
  const shipmentTotal = (timeSlotSuggestions as any[]).reduce((s: number, item: any) => s + item.amount, 0);
  let cumulativeShipment = 0;
  let cumulativeSales = 0;
  const remainSlotValues = ALL_SLOTS.map((slot, idx) => {
    const slotShipment = (timeSlotSuggestions as any[]).filter((s: any) => s.timeSlot === slot).reduce((sum: number, s: any) => sum + s.amount, 0);
  ...
  const displaySlotValues = ALL_SLOTS.map((slot) => {
    const slotAmt = (timeSlotSuggestions as any[]).filter((s: any) => s.timeSlot === slot).reduce((sum: number, s: any) => {
  ...
  const displayTotal = (products as any[]).reduce((sum: number, p: any) => sum + p.displayFullQuantity * p.price, 0);
  ...
  for (const tp of tastingProducts) {
    ...
    for (const slot of activeSlots) {
      const sales = (timeslotSalesRecords as any[]).filter((r: any) => r.timeSlot === slot && r.productName.includes(tp.keyword)).reduce((sum: number, r: any) => sum + r.avgQuantity * (priceMap.get(r.productName) ?? 0), 0);
  ...
  for (const tp of tastingProducts) { const match = (productSuggestions as any[]).find((p: any) => p.productName.includes(tp.keyword)); tastingPriceMap[tp.name] = match?.price ?? 0; }
