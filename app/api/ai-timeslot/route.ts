  ...
  try {
    ...
    if (USE_DB_PROMPT) {
      try {
        ...
        prompt = buildFallbackTimeslotPrompt(dayType, dayTypeLabels, timeSlots, productInfo, historicalContext, hasHistoricalData);
      }
    } else {
      systemInstruction = "你是一个马来西亚烘焙店的排产专家，擅长根据历史销售规律制定分时段出货计划。请严格按照用户要求的 JSON 格式返回分析结果，不要输出任何非 JSON 内容。";
      prompt = buildFallbackTimeslotPrompt(dayType, dayTypeLabels, timeSlots, productInfo, historicalContext, hasHistoricalData);
...
function buildFallbackTimeslotPrompt(
  dayType: string, dayTypeLabels: Record<string, string>, timeSlots: string[],
  productInfo: string, historicalContext: string, hasHistoricalData: boolean
): string {
  return `当前日期类型：${dayTypeLabels[dayType] || dayType}
可用时段：${timeSlots.join(", ")}
